...
async def start(update, context):
    ...
    await update.message.reply_text(
        ...
        f"Лимит: {FREE_DAILY_ANALYSES}/сутки.\n"
        "Кидай фото.",
...
async def ask_for_photo(query):
    await query.message.reply_text(
        "Кидай фото лица.\n"
        "Лучше: ровный свет, лицо в кадре, камера на уровне глаз."
    )
...
async def button_handler(update, context):
    ...
    if query.data == "check_sub":
        if await check_subscription(uid, context):
            await query.message.reply_text("Подписка ок.")
            keyboard = [
                [InlineKeyboardButton("Анализировать фото", callback_data="analyze")],
                [InlineKeyboardButton("Мой лимит", callback_data="limit")],
                [InlineKeyboardButton("Полный анализ — 100 Stars", callback_data="full")],
            ...
            await query.message.reply_text("Ещё не подписан.")
        return
...
async def successful_payment(update, context):
    ...
    if not path:
        await update.message.reply_text("Оплата ок, но фото нет. Кинь ещё раз.")
        return
    await update.message.reply_text("Оплата получена. Полный разбор...")
    try:
...
async def handle_photo(update, context):
    ...
    if used >= FREE_DAILY_ANALYSES:
        ...
        await update.message.reply_text(
            f"Лимит кончился ({FREE_DAILY_ANALYSES}/сутки).",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return

    await update.message.reply_text("Считаю PSL...")
    ...
    try:
        ...
        keyboard = [[InlineKeyboardButton("Полный анализ — 100 Stars", callback_data="full")]]
        await update.message.reply_text(
            "Нужны рекомендации — полный разбор.",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    except Exception as e:
        print(e)
